...
import {
  ...
  Bell, LogOut, Newspaper, Briefcase,
  FileText, Menu, Globe, BookMarked, ShieldCheck,
} from "lucide-react";
...
const navSections = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: BookMarked, label: "Staff Manuals", href: "/dashboard/manuals" },
    ],
  },
  {
    title: "People",
...
export default function InternalLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: myPermissions } = trpc.permissions.myPermissions.useQuery(undefined, {
    enabled: isAuthenticated,
  });
      ...
      <ScrollArea className="flex-1 min-h-0 py-2">
        {navSections.map((section) => (
          <div key={section.title} className="mb-1">
            ...
            {/* Roles & Permissions — only shown to users with manage_users */}
            {section.title === "Administration" && canManageUsers && (() => {
              const href = "/dashboard/admin/permissions";
              ...
              return (
                ...
                >
                  <ShieldCheck className={cn("w-4 h-4 shrink-0", isActive ? "" : "group-hover:text-sidebar-primary")} />
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex w-60 shrink-0 flex-col border-r border-border h-screen overflow-hidden">
        <Sidebar />
      </aside>
