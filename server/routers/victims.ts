...
import { createVictim, getVictims, updateVictim, logActivity } from "../db";
...
export const victimsRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (!hasPermission(daRole, "view_victims")) throw new TRPCError({ code: "FORBIDDEN" });
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const daRole = ctx.user.daRole as any;
      if (!hasPermission(daRole, "edit_victims")) throw new TRPCError({ code: "FORBIDDEN" });
